...
# Obtener la ruta absoluta del directorio actual donde se encuentra trasladoP.py
...
from tools.complemento_pagos.trasladoP import transform_jsonrow as tjr
from config.constant.complemento_pagos import trasladoP_vars as tpv
...
query = tpv.query
cols_= tpv.cols_
output_file = tpv.output_file
regular_exp = tpv.regular_exp
table_name = gv.tables_dest["trasladoP"]
schema = gv.schema

#!#################################################################################################################

# #*##################################GuardarData frame procesado en local##########################################
def trasladoP(next_step, pagos_df,carpeta):
    start = datetime.now()
    print("_"*80)
    print("\/"*60)
    print("="*80)
    ...
    if next_step:
        ...
        traslados_df = tjr.functionThree_TrasladosPCF(cpagos_df)
        traslado = tjr.functionTwo_TrasladoPCF(traslados_df)
        traslado_df = pd.DataFrame(traslado)
        print(traslado_df.shape)
        traslado_reb_df = re_build_df(traslado_df, tpv.ordered_cols, tpv.float_cols,
                                    tpv.str_cols, tpv.int_cols, new_name_tfduuid=tpv.new_name_tfduuid,
                                    to_delete=tpv.to_delete)

        print(traslado_reb_df.head(3))
        print(traslado_reb_df.shape)
        
        name_path = tpv.csv_file_path_to_post
        ...
        print("==> ==> ==> No data to process on TrasladoP <== <== <==")
